...
from .utils.concept_normalizer import normalize_concept, detect_domain, CANONICAL_VOCABULARY
...
def _get_paper_title(md_file: Path) -> str:
    """Extract the real paper title from YAML frontmatter, fallback to slug."""
    try:
        content = md_file.read_text(encoding="utf-8")
    ...
    for concept, papers in sorted(concept_index.items()):
        ...
        if hub_path.exists():
            ...
            new_content = _HUB_TEMPLATE.format(
                ...
                paper_links="\n".join(
                    f"- [[{p}|{_get_paper_title(concepts_dir.parent / '_sources' / (p + '.md'))}]]"
